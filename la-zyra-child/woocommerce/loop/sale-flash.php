...
if( $product->is_on_sale() ){
	...
	$message = esc_html_x( 'Sale!', 'front-view', 'la-zyra' );
	...
	if ( $product->has_child() ) {
		...
		$text = apply_filters('la_zyra/filter/product/sale_badge_title', esc_html_x( 'Save up to', 'front-view', 'la-zyra' ), $product);
		...
		$text = apply_filters('la_zyra/filter/product/sale_badge_title', esc_html_x( 'Save', 'front-view', 'la-zyra' ), $product);
	...
	if( $save_percentage > 0 && $saving_price > 0 ) {
		...
		$saving_price_r = apply_filters('la_zyra/filter/product/sale_badge_price', sprintf( _x( ' %s', 'front-view', 'la-zyra' ), $saving_price ), $saving_price, $product);
		$message .= sprintf('<span class="save-total"><span class="hidden">%s</span>%s</span>', $text, $saving_price_r );
	}
	if( $min_price > 0 ){
		$message .= sprintf('<span class="save-sale-price"><span class="hidden">%s</span>%s</span>', esc_html_x( 'Only', 'front-view', 'la-zyra' ), wc_price( $min_price ) );
	}
