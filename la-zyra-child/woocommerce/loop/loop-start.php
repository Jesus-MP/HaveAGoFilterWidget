...
$active_shop_masonry = LA_Zyra()->settings->get('active_shop_masonry', 'off');
$shop_masonry_column_type = LA_Zyra()->settings->get('shop_masonry_column_type', 'default');
$woocommerce_shop_masonry_columns = LA_Zyra()->settings->get('woocommerce_shop_masonry_columns');

$product_masonry_item_width = LA_Zyra()->settings->get('product_masonry_item_width', 270);
$product_masonry_item_height = LA_Zyra()->settings->get('product_masonry_item_height', 450);
$woocommerce_shop_masonry_custom_columns = LA_Zyra()->settings->get('woocommerce_shop_masonry_custom_columns');
$shop_masonry_item_setting = LA_Zyra()->settings->get('shop_masonry_item_setting');

$item_gap = LA_Zyra()->settings->get('shop_item_space', 'default');

if($item_gap == 'zero'){
    $item_gap = 0;
}

$column_tmp = LA_Zyra()->settings->get('woocommerce_shop_page_columns');
...
$design = LA_Zyra()->settings->get("shop_catalog_grid_style", '1');
...
if($is_main_loop && $active_shop_masonry == 'on'){
    la_zyra_set_wc_loop_prop('prods_masonry', true);
    ...
    if( $shop_masonry_column_type != 'custom' ){
        ...
        if(!empty($shop_masonry_item_setting) && is_array($shop_masonry_item_setting)){
            ...
            foreach($shop_masonry_item_setting as $k => $size){
                $tmp_size = array();
                foreach($size as $_k => $_size){
                    if($_k == 'width') $_k = 'w';
                    if($_k == 'height') $_k = 'h';
                    $tmp_size[$_k] = $_size;
                }
                $__new_item_sizes[$_new_idx] = $tmp_size;
                if(!empty($size['image_size'])){
                    $__new_item_sizes[$_new_idx]['s'] = LA_Zyra_Helper::get_image_size_from_string($size['image_size']);
                }
                $_new_idx++;
            }
        }
        la_zyra_set_wc_loop_prop('item_sizes', $__new_item_sizes);
        $masonry_component_type = 'AdvancedMasonry';
    }
    la_zyra_set_wc_loop_prop('image_size', LA_Zyra_Helper::get_image_size_from_string(LA_Zyra()->settings->get('product_masonry_image_size', 'shop_catalog')));
    ?>
