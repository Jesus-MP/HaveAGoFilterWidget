	...
	<table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo $wishlist_id ?>" data-token="<?php echo $wishlist_token ?>">
        ...
        <tr>
            ...
            <?php if( $show_price ) : ?>
                ...
                <th class="product-price" data-title="Unit Price">
                    <span class="nobr">
			...
			<!-- Speed -->
			<th class="product-speed" data-title="Speed">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_speed_heading', __( 'Speed', 'yith-woocommerce-wishlist' ) ) ?>
				</span>
            </th>
			
			<!-- Distance -->
			<th class="product-distance" data-title="Distance">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_distance_heading', __( 'Distance', 'yith-woocommerce-wishlist' ) ) ?>
                    </span>
                </th>
			
			<!-- Weight -->
			<th class="product-weight" data-title="Weight">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_weight_heading', __( 'Weight', 'yith-woocommerce-wishlist' ) ) ?>
                    </span>
                </th>
            ...
            <?php if( $show_stock_status ) : ?>
                ...
                <th class="product-stock-status" style="display: none;">
                    <span class="nobr">
                    ...
                    <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
                        ...
                        <?php if( $show_price ) : ?>
                            <td class="product-price">
                                ...
                                echo $base_product ? $product->get_price_html() : apply_filters( 'yith_free_text', __( 'Unknown', 'yith-woocommerce-wishlist' ) ); 
                                ?>
                            </td>
...
<!-- function to get the speed weight and distance from product id-->
						<?php
			/* variable access the postmeta data base */
			global $wpdb;
						
			/* product id and meta key used to pull meta value from data */			
			$p_id =$product->get_id();
						
			$meta_key=array("product_speed","product_distance","product_weight");
			$meta_title=array("Speed", "Distance", "Weight" );
			...
			$meta_values[$x] = $wpdb->get_var("SELECT meta_value FROM $wpdb->postmeta WHERE post_id=$p_id AND meta_key='$meta_key[$x]'");
							$x++;
			$meta_values[$x]= $wpdb->get_var("SELECT meta_value FROM $wpdb->postmeta WHERE post_id=$p_id AND meta_key='$meta_key[$x]'");
			$x++;
			$meta_values[$x] = $wpdb->get_var("SELECT meta_value FROM $wpdb->postmeta WHERE post_id=$p_id AND meta_key='$meta_key[$x]'");			
			...
			?>
						...
						<!-- loops through meta values and prints them out !-->
						<?php

						
						foreach($meta_values as $index => $values)
						{
							...
							if ($values=='')
							{
							  ...
							  echo '<td class="'.$meta_key[$index].'" data-title="'.$meta_title[$index].'">'.$values. '</td>';	
                        ...
                        <?php if( $show_stock_status ) : ?>
                            <td class="product-stock-status" style="display: none;">
